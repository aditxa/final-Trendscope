...
export function useTrends() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return parseWithLogging(api.trends.list.responses[200], data, "trends.list");
    },
  });
}
...
export function useRunPipeline() {
  ...
  return useMutation({
    mutationFn: async (input: { source: string }) => {
      ...
      const data = await res.json();
      return parseWithLogging(api.trends.runPipeline.responses[200], data, "trends.pipeline");
    },
    onSuccess: () => {
